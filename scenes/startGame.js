...
var startTen = this.add.bitmapText(game.config.width / 2 - 50, 675, 'topaz', 'Ten Pair', 50).setOrigin(0, .5).setTint(0x000000);
	...
	var startNumberRPG = this.add.bitmapText(game.config.width / 2 - 50, 1275, 'topaz', 'NumberRPG', 50).setOrigin(0, .5).setTint(0x000000);
    startNumberRPG.setInteractive();
    startNumberRPG.on('pointerdown', function(){
		this.clickHandler(9)
	}, this);
...
//var pimage = this.add.image(500,500, 'plus').setScale(.2)
  ...
  clickHandler(cg)
  {
    ...
    this.scene.launch('nonoOptions');
      ...
      this.scene.pause();
    this.scene.launch('minesweeperOptions');
    }else{
      currentGame = cg;
